...
/*        KeyStore keystore;
    ...
    private OCSPReq getOCSPRequest(X509Certificate caCert, BigInteger revokedSerialNumber) throws Exception {
        ...
        return ocspVerifier.generateOCSPRequest(caCert, revokedSerialNumber);
    }
    ...
    public void runOcspButton() throws Exception {
        ...
        final BasicOCSPResp basicResponse = (BasicOCSPResp) ocspResponse
                .getResponseObject();
        if (basicResponse==null || basicResponse.getResponses().length < 1) {
        ...
        if(!request.getExtension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce).equals(basicResponse.getExtension(OCSPObjectIdentifiers.id_pkix_ocsp_nonce))
        ) {
            responseTextArea.setText("OCSPObjectIdentifiers.id_pkix_ocsp_nonce запроса и ответа не совпадает");
        }
...
/*        X509Certificate x509Certificate = new JcaX509CertificateConverter().setProvider("BC")
        ...
        ocspTempLocation = "temp/" + MeUtils.getThumbprint(x509CertificateHolder.getEncoded()) + ".cer";
        File file = new File(ocspTempLocation);
        FileManager.writeWithDir(file, x509CertificateHolder.getEncoded());
    }

    @FXML
    public void showOcspCertButton() throws IOException {
/*
