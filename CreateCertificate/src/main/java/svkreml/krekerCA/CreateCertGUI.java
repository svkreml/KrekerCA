...
public class CreateCertGUI {
    ...
    public Tab initCreator() {
        ...
        Button loadCert = new Button("Загрузить поля из сертификата");
        loadCert.setPadding(new Insets(5, 10, 5, 10));
        loadCert.setAlignment(Pos.CENTER_RIGHT);
        loadCert.setOnAction(this::loadCertificate);
        ...
        bottomMenu.setPadding(new Insets(5, 10, 5, 10));
        bottomMenu.getChildren().addAll(createCert, loadCert);
        tabRootBox.getChildren().add(bottomMenu);
        createCertTab.setContent(tabRootBox);

        return createCertTab;
    }
    private void loadCertificate(ActionEvent event) {
        System.out.println("loadCertificate");
        try {
            final String path = JavaFxUtils.fileChooser("Выбор сертификата для импорта").getAbsolutePath();
            certificateGeneratorHandler.loadCertificate(path, serialTF, dateFromDP,
                    dateToDP, algsCB, subjectFields, extensionFields, certPath.getSelfSigned(),certPath.getCaCertificateTF());
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private int setExtensions(int row) {
        ...
        extensionFields.add(new CaVersionExtensionField());
