...
public class SubjectSignToolObject implements ExtensionObject {
    private String[] texts;
    private boolean isCritical;
    public SubjectSignToolObject(boolean isCritical, String[] lines) {
        this.texts = lines;
    ...
    public void addExtension(CertBuildContainer certBuildContainer) throws IOException {
        ...
        if (texts.length == 1) {
            certBuildContainer.getX509v3CertificateBuilder().addExtension(CustomExtension.subjectSignTool, isCritical, new DERUTF8String(texts[0]));
        } else if (texts.length > 1) {
            ASN1EncodableVector lines = new ASN1EncodableVector();
            for (int i = 0; i < texts.length; i++) {
                lines.add(new DERUTF8String(texts[i]));
            }
            certBuildContainer.getX509v3CertificateBuilder().addExtension(CustomExtension.subjectSignTool, isCritical, new BERSequence(lines));
        }
    }
}
